...
func TestVector_ECDH(t *testing.T) {
	var (
		...
		want      = mustDecodeHex("0x033b11a2a1f214567e1537ce5e509ffd9b21373247f2a3ff6841f4976f53165e7e")
	)
...
func TestVector_KDF(t *testing.T) {
	var (
		...
		cdata  = mustDecodeHex("0x000000000000000000000000000000006469736376350001010102030405060708090a0b0c00180102030405060708090a0b0c0d0e0f100000000000000000")
	...
	check(t, "initiator-key", s.writeKey, mustDecodeHex("0xdccc82d81bd610f4f76d3ebe97a40571"))
	check(t, "recipient-key", s.readKey, mustDecodeHex("0xac74bb8773749920b0d3a8881c173ec5"))
}
...
func TestVector_IDSignature(t *testing.T) {
	var (
		...
		ephkey = mustDecodeHex("0x039961e4c2356d61bedb83052c115d311acb3a96f5777296dcf297351130266231")
		cdata  = mustDecodeHex("0x000000000000000000000000000000006469736376350001010102030405060708090a0b0c00180102030405060708090a0b0c0d0e0f100000000000000000")
	)
	...
	check(t, "id-signature", sig, mustDecodeHex(expected))
}
...
func hexPubkey(curve elliptic.Curve, input string) *ecdsa.PublicKey {
	key, err := DecodePubkey(curve, mustDecodeHex(input))
	if err != nil {
		panic(err)
	}
	return key
